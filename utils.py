...
def sp_getView(num = None):
	...
	con = pyodbc.connect(
			'DRIVER={SQL Server};'+
			'SERVER=192.168.0.2\sqlexpress2008;'+
			'DATABASE=cc_temp;'+
			'UID=VPetrov;'+
			'PWD=h44ruMnSNr'
	)
...
def getActiveTaskList():
	...
	q = """
		SELECT p.project_id num, p.project_name name
		FROM [CATI4].[dbo].[vw_OTask] ot
		join [CATI4].[dbo].mkt_project p
			on p.Project_OL = ot.ID
		where ot.active = 1
	"""
	cur.execute(q)
